document.addEventListener('DOMContentLoaded', function () {
    ...
    function renderDepartments(departments) {
        ...
        departments.forEach(department => {
            ...
            departmentCard.className = 'text-center department-card col-12 col-md-6 col-lg-4 mb-4 d-flex align-items-stretch';

            const description = department.description || 'No hay descripción disponible.';
            const imageUrl = departmentImages[department.name.toLowerCase().replace(/\s+/g, '_')] || 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/dd/Colombia_in_its_region_%28San_Andres_and_Providencia%29.svg/1920px-Colombia_in_its_region_%28San_Andres_and_Providencia%29.svg.png'; // URL de imagen por defecto

            departmentCard.innerHTML = `
                <div class="card h-100 w-100">
                    <img src="${imageUrl}" class="card-img-top custom-img" alt="${department.name}">
                    <div class="card-body">
                        <h3 class="card-title">${department.name}</h3>
                        <p class="card-text">${description}</p>
                        <a href="./details.html?id=${department.id}" class="btn btn-primary">Details</a>
                </div>
    ...
    fetch('https://api-colombia.com/api/v1/Department')
        .then(response => response.json())
        .then(data => {
            // Guardar los departamentos en una variable para el filtrado
            const departments = data;

            // Renderizar departamentos inicialmente
            renderDepartments(departments);

            // Filtro de departamentos
            filterInput.addEventListener('input', function () {
                const filterText = normalizeString(filterInput.value.toLowerCase());
                const filteredDepartments = departments.filter(department =>
                    normalizeString(department.name.toLowerCase()).includes(filterText)
                );
